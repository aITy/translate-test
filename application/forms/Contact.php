...
//                ->setRequired(true);
        ...
        //textarea
        $messageContent = new Zend_Form_Element_Textarea('message_content');
        //atributy textarea (pocet radku/sloupcu)
        $messageContent->setAttribs(array('cols' => 60, 'rows' => 6));
        //nastaveni zobrazovaneho popisku
        $messageContent->setLabel('Obsah zprávy');
        //je vyzadovan
        $messageContent->setRequired(true);

        $control = new Zend_Form_Element_Text('control');
...
//                ));
        ...
        $this->setAction($url->url(array('controller' => 'index', 'action' => 'send'), 'default'));
    }
